...
def alignment(pep_dict, nuc_dict, pair, out):
    id = "_".join(pair)
    fasfile = id + ".fasta"
    pepaln = id  + "." + "aln"
...
for p in genepair:
    ...
    aln_name = alignment(pseqs_dict, nuc_dict, p, out)
    os.system('KaKs -i %s/%s -o %s/%s.kaks -m YN > %s/%s.log' % (out, aln_name, out, aln_name, out, aln_name))
