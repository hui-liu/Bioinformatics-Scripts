...
def cds_blast(gene, cds_fasta, DB):
    # make a temp file
    aa = "_".join(os.path.basename(output_file).split("_")[:2])
    gene_file = tempfile.NamedTemporaryFile(suffix="_"+aa+"_temp.txt",
                                            prefix= "gene_",
                                            mode='r+',
                                            delete=False,
                                            )
        ...
        if "C" in Blast[0]:
            ...
            if len(Syn_genes) > 1:
                ...
                for idx, gene in enumerate(Syn_genes):
                    ...
                    #2020/07/15: with newest version of gene annotation the gene numbers are 100 apart, instead of 1
                    #So I changes the int(coordinates[idx]) - int(coordinates[idx-1]) to < 300 from < 3
                    if idx > 0:
                        if int(coordinates[idx]) - int(coordinates[idx-1]) < 1100:
