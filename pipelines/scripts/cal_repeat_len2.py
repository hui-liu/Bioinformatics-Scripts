...
for chr in TandemRepeat:
    merged_reg = merge_intervals(TandemRepeat[chr])
    length = sum([i[1] - i[0] + 1 for i in merged_reg])
    len_TandemRepeat.append(length)
...
print("TandemRepeat", sum(len_TandemRepeat), round(sum(len_TandemRepeat) * 100.0 / size, 2))
