...
def main(inTrf):
	...
	for recs in trf_reader.parse():
		...
		for record in recs.records:
			print recs.id, record.start, record.end, record.start/100000 * 100000, record.period_size, record.consensus_size, record.copy_number, record.tr_length, record.motif, record.seq
